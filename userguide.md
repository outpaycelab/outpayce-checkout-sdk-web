...
```HTML
<script src="https://prd.payment.amadeus.com/checkout/sdk/6.2.0/sdk-es2022.js" integrity="sha384-I9O/ft/euAIK714fSE98kVbpd2UfFxpxpO/VsNTOjiXEUivFFb4806EV3DxKUmFk" crossorigin="anonymous"></script>
<link rel="stylesheet" href="https://prd.payment.amadeus.com/checkout/sdk/6.2.0/sdk.css" integrity="sha384-UllymVRYIljeXwI0hy6AyiCHNb9pHV/7SV3yASh9ThvMoyYf8QhHaDMYYN/wcqDK" crossorigin="anonymous">
```
...
```HTML
<link rel="stylesheet" href="https://prd.payment.amadeus.com/checkout/sdk/6.2.0/sdk-no-material.css" integrity="sha384-X3kDRaqqSJmv86WyKNWaYsK19nAaXeO4wZmacpMwGeh6xrdvBt9bIqSVok5kbTzi" crossorigin="anonymous">
```
...
|`iframeStyle`| {[key: string]: string} | `{}` | Allows for the application of custom CSS styles directly to payment form fields within iframes. This object accepts key-value pairs where the key is a CSS property and the value is the style setting. You can customize `fontSize`, `backgroundColor`, `paddingLeft`, `paddingRight`, `textAlign`, `fontFamily` to ensure the payment form aligns seamlessly with the visual aesthetics of your website or application.
If needed, a custom font can be added thanks to `fontUrl`. In case a `fontUrl` is provided, it is advised to pass a `fontFamily` with a fallback value: `"'customFont', 'Helvetica Neue', sans-serif"` -> `fontUrl` will be associated to customFont and in case it fails to load, it will default to Helvetica Neue.
The provided url must have a `Access-Control-Allow-Origin` header allowing our domain and your domain must be whitelisted on our side (contact the Amadeus Payment implementation team).
...
Some payment methods support promotional widgets (e.g., Tabby ) that are configured via the `WidgetManagement` property in the profile manager. The SDK dynamically loads and displays these widgets alongside the payment form.

**Configuration properties:**
- `displayWidget`: Enable/disable widget display
- `publicKey`: Merchant's public key for the widget provider
- `merchantCode`: Merchant identifier
- `widgetURLs`: Array of widget script URLs
...
| AmopSwish              | Swish                        |                                                                                                                                                       |
| AmopTamara             | Tamara                       |                                                                                                                                                       |
| AmopTabby              | Tabby                        | Supports promotional widget display when configured via `WidgetManagement` in the profile manager.
| AmopTMoney             | T Money                      |                                                                                                                                                       |
